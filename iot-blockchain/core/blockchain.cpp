...
Policy(blocks->at(0).get()->getTransactionAt(0)->getAuthorProfileIdentifier()) // genesis transaction always declares admin profile identifier
{
	...
	for (auto &block : *blocks)
	{
		...
		{
			this->submitTransaction(block->getTransactionAt(i), true);
		}
	}
}
...
//	if (block->getIndex() == lead_block->getIndex() && block->size() >= lead_block->size())
//	{
//		for (size_t i = lead_block->size(); i < block->size(); i++)
//		{
//			this->submitTransaction(block->getTransactionAt(i));
//		}
//
//		this->blocks.at(block->getIndex()) = std::make_unique<Block>(block);
//	}
	
//	else if (block->getIndex())
//
...
void Blockchain::spawnNewBlock(bool suppress)
{
	...
	{
		this->sealed_block_responder(this->getBlockAt(this->blocks.size() - 2)); // fire sealed block
	}
}
...
JSON Blockchain::json(void)
{
	JSON json;
	
	for (auto i = 0; i < this->blocks.size(); i++)
	{
		json.push_back(this->blocks.at(i)->json());
	}
